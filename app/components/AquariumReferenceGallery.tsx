...
import type { GalleryGroup, GalleryItem } from "../data/reeferG3MaxGallery";

type LightboxState = { item: GalleryItem; groupSlug: string };

export function AquariumReferenceGallery({
  title,
  groups,
}: {
  title: string;
  ...
  function navigate(delta: number) {
    ...
    const group = groups.find((g) => g.items.some((i) => i.id === next.id));
  ...
  return (
    <div className="mt-16 border-t border-slate-700 pt-14">
      <div className="mb-8">
        ...
        <h2 className="mt-2 text-xl font-semibold tracking-tight text-slate-300">{title}</h2>
